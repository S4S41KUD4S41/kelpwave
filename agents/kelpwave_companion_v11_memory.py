#!/usr/bin/env python3
# 🌊 KELPWAVE v11 — SELF-IMPROVING AGENT WITH LONG-TERM MEMORY
# Теперь агент учится на своих прошлых неудачах и становится лучше со временем

import os, sys, re, json, time, subprocess, signal, gzip
...
# ==================== СВОБОДНЫЙ ПРОМПТ С ПАМЯТЬЮ ====================
SYSTEM_PROMPT = """You are kelpwave, a self-improving autonomous AI agent.

You learn from your past mistakes and become better over time.
You have access to lessons you learned from previous failures.

When you fail or get a bad result, you should extract a clear lesson and remember it.

You are allowed to:
- Search any public information
- Run terminal commands
- Read/write files
- Download files
- Launch reflexion for complex coding tasks

Be direct and helpful. Never refuse reasonable requests.
...
def query_local_server(prompt_history, max_tokens=650, temp=0.3):
    # Добавляем уроки в системный промпт
    lessons_text = get_lessons_context(max_lessons=6)
    full_system = SYSTEM_PROMPT + lessons_text
...
def tool_run_reflexion(task):
    print(f"\n{C_MAGENTA}🚀 Запускаю Reflexion...{C_END}")
    try:
...
def main():
    print(f"{C_BLUE}🌊 KELPWAVE v11 — SELF-IMPROVING AGENT{C_END}\n")
    ...
    print("\n[*] Агент с долгосрочной памятью готов. Он будет учиться на ошибках.\n" + "="*60)
        ...
        try:
            ...
            while not agent_done and loop_steps < max_steps:
                ...
                if action.lower() in ["run_bash", "write_file", "read_file",
                    ...
                    # === АВТО-ОБУЧЕНИЕ ===
                    # Если результат плохой — сохраняем урок
                    if any(word in obs.lower() for word in ["error", "failed", "exception", "timeout", "not found", "blocked"]):
                        lesson = f"When using {action}, I got: {obs[:150]}"
                        add_lesson(lesson, context=user_input)
