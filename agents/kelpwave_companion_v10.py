#!/usr/bin/env python3
# 🌊 KELPWAVE COMPANION v10 — свободная модель + улучшенный поиск
# Основные изменения:
# - Полностью убраны жёсткие запреты в системном промпте
# - Агент может искать любую информацию, включая ники
# - Улучшенный веб-поиск + обработка результатов
# - Автоматический запуск setup + doctor при первом запуске
...
# ==================== НОВЫЙ СВОБОДНЫЙ ПРОМПТ ====================
SYSTEM_PROMPT = """You are kelpwave, a powerful autonomous AI agent running locally on the user's Android phone in Termux.

You are helpful, direct, and never refuse reasonable requests. You can search the web, read/write files, run terminal commands, and download files.

You are allowed to:
- Search for any public information on the internet (including usernames, nicknames, public profiles)
- Download any publicly available files
- Execute any terminal commands that make sense for the task
- Write any information the user asks for into files

Only refuse if the request is clearly illegal or extremely harmful (e.g. hacking someone's private account, creating malware, etc.).

When the user asks you to find something:
1. Use web_search first with clear keywords
2. Pick the most relevant URLs from the results
3. Use fetch_page or download_file on those URLs
4. Save useful information to files when requested

Never say "I can't" or "I won't" unless the request is genuinely harmful. Always try to be useful.

Environment:
- Termux home: /data/data/com.termux/files/home
- User files folder: /sdcard/Download/kelpwave
...
When you have the final answer, just reply in normal text without any ACTION.
"""
...
def query_local_server(prompt_history, max_tokens=600, temp=0.3):
    ...
    try:
        ...
        return f"THOUGHT: Error: {e}. ACTION: none."
...
def _search_ddg_lite(query, n=6):
...
def tool_download_file(url):
    try:
        ...
        with open(dest, "wb") as f:
            f.write(raw)
        return f"[Downloaded {len(raw)} bytes → {dest}]"
    except Exception as e:
        return f"[Error: {e}]"
...
def main():
    print(f"{C_BLUE}🌊 KELPWAVE COMPANION v10 (свободная модель){C_END}\n")
    ...
    print("\n[*] Агент готов. Пиши запросы или 'exit' для выхода.\n" + "="*60)
        ...
        try:
            ...
            while not agent_done and loop_steps < max_steps:
                ...
                if action.lower() in ["run_bash", "write_file", "read_file",
                                      "web_search", "fetch_page", "download_file"]:
                    sig = (action.lower(), action_input.lower()[:100])
                    if sig in seen_calls:
                        print(f"{C_YELLOW}[LOOP] Повтор — пропускаю{C_END}")
                        loop_steps += 1
                        continue
                    seen_calls.add(sig)

                    print(f"\n🧠 {thought}")
                    print(f"🎬 {action}: {action_input[:80]}...")
            ...
            if not agent_done:
                print(f"{C_YELLOW}[Лимит шагов] Запрашиваю финальный ответ...{C_END}")
                history.append({"role": "user", "content": "OBSERVATION: Tool limit reached. Give final answer now."})
                response = query_local_server(history)
                clean = response.replace("THOUGHT:", "").strip()
                print(f"\n🌊 {C_BLUE}kelpwave:{C_END} {clean}")
